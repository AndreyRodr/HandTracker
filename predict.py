...
try:
    ...
    print("[ERRO] Arquivos do modelo não encontrados. Execute 'python src/train.py' primeiro.")
    exit()


# Inicializando a solução MediaPipe Hands.
# ATENÇÃO: Apenas a detecção de MÃOS é usada no projeto (21 landmarks)
...
CONFIDENCE_THRESHOLD = 0.8 # Confiança mínima para uma predição ser considerada válida.
...
while True:
    ...
    if result.multi_hand_landmarks:
        ...
        # 1. Extraindo as coordenadas brutas (63 valores).
        ...
        # NOTE: O modelo Scikit-learn usa predict_proba, não o predict do Keras/TensorFlow.
    ...
    # Encontrando a predição mais comum no histórico.
    if valid_preds:
    ...
    if current_stable_letter:
        display_text = f"PREVISÃO: {current_stable_letter}"
        if letter_confirmed:
        ...
        display_text = "PREVISÃO: Aguardando gesto"
    
    # Desenhando o status e a frase.
    ...
    cv2.putText(frame, f"PALAVRA: {' '.join(sentence)}", (20, H - 20), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 3, cv2.LINE_AA)
